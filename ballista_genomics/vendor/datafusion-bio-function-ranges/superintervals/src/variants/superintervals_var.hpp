...
//    inline void upperBoundRange(const S value, size_t& left, const size_t right) noexcept {
...
//            length = half;
//        }
//        if (starts[left] >= value) {
//            --left;    // underflows to SIZE_MAX
//        }
//    }
//
//    void findOverlaps2(const S start, const S end, std::vector<T>& found) {
//        if (starts.empty()) {
//            return;
//        }
//        upperBound(end);
...
//        }
//
//        size_t starting_idx = idx;
//        size_t right = idx;
//        size_t bound = 1;
//
//        // Exponential search to find range where start < starts[idx] stops being true
//        while (idx > 0 && start < starts[idx]) {
//            right = idx;
//            idx = (bound <= idx) ? idx - bound : 0;
//            bound *= 2;
//        }
//
//        // Binary search in range [idx, right] to find exact boundary
//        upperBoundRange(start, idx, right);
//        if (idx == SIZE_MAX) {
//            found.insert(found.end(), data.rend() - starting_idx - 1, data.rend());
//            return;
//        }
//        // Bulk insert from starting_idx down to idx
//        found.insert(found.end(), data.rend() - starting_idx - 1, data.rend() - idx - 1);
//
//        // Continue with branching logic
//        while (idx != SIZE_MAX) {
//            if (start <= ends[idx]) {
//                found.push_back(data[idx]);
//                --idx;
//            } else {
//                idx = branch[idx];
//            }
...
//    }
    ...
    inline void upperBoundRange(const S value, size_t& left, const size_t right) noexcept {
        ...
        // Now do binary search in the reduced range [left, search_right]
    ...
    size_t countOverlaps2(const S start, const S end) noexcept {
        if (starts.empty()) {
            return 0;
        }
        upperBound(end);
